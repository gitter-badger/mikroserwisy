...
internal sealed class CustomMessageSerializationStrategy : IMessageSerializationStrategy
{
    ...
    public IMessage DeserializeMessage(MessageProperties properties, in ReadOnlyMemory<byte> body)
    {
        ...
        if (type is null)
        {
            throw new InvalidOperationException($"Message was not registered for type: '{properties.Type}'.");
        }
        ...
        _contextAccessor.Context = new Context(activityId, traceId, properties.MessageId, causationId, userId);
